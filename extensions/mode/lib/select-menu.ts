...
function activeMenu(
	...
	return {
		render(width: number): string[] {
			...
			const row = (content: string) => theme.fg("border", glyph.vertical)
				+ truncateToWidth(content, inner, "…", true)
				+ theme.fg("border", glyph.vertical);
			...
			if (matches.length === 0) {
				lines.push(row(` ${theme.fg("warning", "No matches")}`));
			} else {
				for (let index = start; index < Math.min(matches.length, start + visibleCount); index += 1) {
		...
		handleInput(data: string): void {
			...
			else if (filterable && matchesKey(data, Key.backspace)) {
