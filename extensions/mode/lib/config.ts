...
export type ModeConfigWriteResult = { ok: true; path: string } | { ok: false; path: string; error: string };

/** Atomically mutate only the mode section in the global terrific config. */
export function updateModeConfig(
	agentDir: string,
	mutate: (mode: Record<string, unknown>) => void,
): ModeConfigWriteResult {
	...
	try {
		...
		if (Object.hasOwn(root, "mode") && !isRecord(root.mode)) {
			return { ok: false, path, error: "mode must be a JSON object" };
		}
		const mode = isRecord(root.mode) ? root.mode : {};
		mutate(mode);
		if (Object.keys(mode).length === 0) delete root.mode;
		else root.mode = mode;
