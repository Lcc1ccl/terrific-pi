...
export function withoutOwnedGlobalConfigWarning(
	warnings: readonly string[],
	profile: AppearanceProfileResult,
	agentDir = resolveAppearanceAgentDir(),
): string[] {
	if (!profile.error) return [...warnings];
	const prefix = `terrific-config: failed to read ${join(agentDir, "terrific.json")}:`;
