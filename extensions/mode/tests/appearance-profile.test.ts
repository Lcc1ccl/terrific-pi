...
function root(): string {
	const value = mkdtempSync(join(tmpdir(), "mode-appearance-"));
	roots.push(value);
	return value;
}
...
describe("global appearance profile", () => {
	it("covers the frozen eight-vector contract", () => {
		const cases: Array<{ source?: string; active: boolean; error: boolean }> = [
			...
			{ source: JSON.stringify({ appearance: "on" }), active: false, error: true },
		...
		const agentDir = root();
		const project = root();
		write(agentDir, JSON.stringify({ appearance: { profile: "off" } }));
