...
- After Appearance successfully owns the editor, it requests a width-aware status renderer over `terrific-pi:statusline:editor-v1`. Enabled `model` (including thinking), `mode`, and `fast` widgets move to the editor's bottom-right border without duplicating in the footer.
- The bridge adds no configuration: `/statusline` still controls widget enablement, order, separator, and icon mode, while the host theme supplies colors. Disabled Appearance or an editor conflict leaves the widgets in the footer.
