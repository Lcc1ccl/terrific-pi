...
- Absent, disabled, or non-TUI configuration has zero UI side effects. Malformed configuration remains fail-closed and emits one warning per extension generation without installing an owner. An existing foreign editor owner makes editor-enabled startup fail closed without installing the header.
