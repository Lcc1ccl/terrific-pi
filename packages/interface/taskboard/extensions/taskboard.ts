...
export default function taskboard(pi: ExtensionAPI) {
	...
	const notifyInvalidShortcut = (ctx: ExtensionContext) => {
		...
		ctx.ui.notify(
			`Invalid taskboard.toggleShortcut "${initialConfig.invalidToggleShortcut}"; using shift+alt+o`,
			"warning",
		);
	};
	...
	const syncDurationTick = () => {
		...
		const timer = setInterval(() => {
			if (durationTickTimer === timer) requestWidgetRender?.();
		}, 1_000);
		durationTickTimer = timer;
	...
	if (initialConfig.toggleShortcut) {
		pi.registerShortcut(initialConfig.toggleShortcut, {
