...
- a live task panel toggled independently with a configurable shortcut (`Shift+Alt+O` by default)
...
Install the `terrific-pi` root package. It preserves the required load order between Taskboard and Presentation:
...
`taskboard.toggleShortcut` accepts a Pi `KeyId` (default `shift+alt+o`) or `off`. Invalid values fall back to the default and warn once. Run `/reload` after changing this value so Pi tears down the previous extension shortcut and registers the new one.
...
```bash
cd packages/interface/taskboard
npm install
npm run check
```
